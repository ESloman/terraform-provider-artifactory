...
### Package Version Pattern Cleanup Policy

```terraform
resource "artifactory_package_cleanup_policy" "my-package-version-policy" {
  key = "my-package-version-policy"
  description = "Cleanup based on package version patterns"
  cron_expression = "0 0 2 ? * MON-SAT *"
  duration_in_minutes = 60
  enabled = true
  skip_trashcan = false
...
```terraform
resource "artifactory_package_cleanup_policy" "time-and-properties" {
...
- `excluded_package_versions` (Set of String) Specify patterns for package versions that you want excluded from the cleanup policy.
...
- `included_package_versions` (Set of String) Specify patterns for package versions on which you want the cleanup policy to run. To include all versions, use `**`. Example: `included_package_versions = ["**"]`
